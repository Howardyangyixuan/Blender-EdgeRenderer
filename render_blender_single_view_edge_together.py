...
bpy.ops.import_scene.obj(filepath=args.edge)
...
for i in range(len(bpy.context.selected_objects)):
    ...
    name = bpy.context.selected_objects[i].name
    if name == "Light" or name=="Camera": continue 
    bpy.context.selected_objects[i].rotation_euler[0] = 0
    bpy.context.selected_objects[i].rotation_euler[1] = 0
    bpy.context.selected_objects[i].rotation_euler[2] = -0.523599

edge = bpy.context.selected_objects[2]
# a little side
...
hold = mat.node_tree.nodes.new('ShaderNodeHoldout')
obj = mat.node_tree.nodes.get('Material Output')
# edge_mat.node_tree.nodes["Holdout"].inputs[0].default_value = (1, 1, 1, 1)
mat.node_tree.links.new(obj.inputs[0], hold.outputs[0])
# mat.node_tree.nodes["Background"].inputs[0].default_value = (1, 1, 1, 1)
...
bpy.ops.object.material_slot_add()
edge.material_slots[0].material = edge_mat
edge_mat.use_nodes = True
edge_node = edge_mat.node_tree.nodes.new('ShaderNodeBackground')
output = edge_mat.node_tree.nodes.get('Material Output')
...
bpy.data.linestyles["LineStyle"].color = (1, 1, 1)
...
for i in range(0, args.views):
    ...
    render_file_path = fp + '_r_{0:03d}_edge'.format(int(i * stepsize))
